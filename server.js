...
async function transitionToReveal(round) {
  ...
  try {
    ...
    // Resolve every bet: safe rooms pay 3x, haunted room pays 0
    await client.query(
      `UPDATE room_bets
  ...
  try {
    ...
    if (IS_STAGING) {
      ...
      if (existingDemo.length === 0) {
        ...
        // Spread demo bets: some on the haunted room (lost), some safe (3x)
        const demoBets = [
          { room: 'library',      safe: true  },
          { room: 'kitchen',      safe: false },
          { room: 'backyard',     safe: true  },
          { room: 'dining-room',  safe: true  },
          { room: 'reading-room', safe: true  }
        ];

        for (let i = 1; i <= 5; i++) {
          const bet = demoBets[i - 1];
          const amount = 100 * i;
          const payout = bet.safe ? amount * PAYOUT_MULTIPLIER : 0;
          const { rows: pr } = await pool.query(
